...
because nothing generates artifact-version receipts.** Writing a receipt file by hand
and asserting the conjunct holds would reproduce, inside the gate meant to catch it,
the defect `AUTO_MERGE_STANDARD.md` §7 records: a schema is not a control. It was the
easiest conjunct to fake and is therefore the one stated as absent.
...
def test_review_revision_is_a_separate_field_from_version():
    ...
    for entry in artifacts():
        ...
        assert meta["review_revision"] != meta["version"], (
            "a review-iteration count and a semantic version must not be the same token"
        )
...
def test_review_revision_is_strictly_increasing_and_current_is_the_latest():
    for entry in artifacts():
        ...
        assert entry["review_revision"] == revs[-1], (
            "the current entry must be the newest revision, or the manifest is describing "
            "a state the tree is not in"
        )
...
def test_receipt_conjunct_is_declared_unsatisfiable_and_nothing_pretends_otherwise():
    """`AMS-03`: no producer generates artifact-version receipts.

    Asserted as behaviour, not as artifact absence — the correction already applied
    to `AMS-02`. A file could be named anything; what matters is that no script or
    workflow *produces* a receipt, so the conjunct cannot be satisfied by a filename.
    """
    ...
    for path in sorted((REPO_ROOT / "scripts").glob("*.py")) + sorted(
        ...
        if "artifact-version-receipt" in text or "artifact_version_receipt" in text:
            producers.append(str(path.relative_to(REPO_ROOT)))
    assert not producers, (
        f"{producers} reference an artifact-version receipt. If a producer now exists, "
        "AMS-03 must flip deliberately: the conjunct moves to ENFORCED in the standard's "
        "§13 table and this test is rewritten to assert the receipt is required"
    )
