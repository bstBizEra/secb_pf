...
| Stage 2 | **PASSED — `APPROVED_WITH_CONDITIONS`**, issued by the operator 2026-08-12T16:36:07Z. 7/7 objectives, 7/7 criteria, obligation posture `OPEN_NON_BLOCKING` | `STAGE_GATE_REQUIREMENTS_READY.md` at `ISSUED`; composite form authorized by ballot 002; `SECB-WP-FWK-056` |
| Stage 3 | **`OPEN`** — all five transition-guard conjuncts hold; it opened because they hold, not because this row says so. Authorizes architecture-design work toward `ARCHITECTURE_APPROVED` and **nothing past it** | Guard evaluated in `STAGE_GATE_REQUIREMENTS_READY.md` §"Transition guard for stage 3" |
