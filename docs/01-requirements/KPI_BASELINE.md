...
| K-05a | **Enforcement coverage** | mechanically fail-able gates / 10 | `ci.yml` versus `CONTROL_GATES.md` | Per change to CI | **4/10 = 40%** — **Authority (1), Test (5), Security (6), Evidence (7)**. Gate 6's two scans (`NFR-17` committed-secret, `NFR-18` prohibited-call) ARE on `main`, in `ci.yml`, landed by `SECB-WP-FWK-048` (#96) and `SECB-WP-FWK-049` (#101). Gate 7 is `SECB-WP-FWK-045`, which recomputes the five sealed digests and fails on any mismatch. **Budget is deliberately NOT counted here**: it is mechanically fail-able, but it is not one of the ten gates `CONTROL_GATES.md` defines, and this row's denominator is that register — a numerator member outside its own denominator is not a coverage figure. That Budget can block a merge while having no home in the mandatory-gate register is a real gap, reported separately. The governance-verdict job remains uncounted — it exits 0 by design | 10/10 | Counting a job that cannot fail would inflate this row | Operator | **Yes** |
